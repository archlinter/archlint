...
**ID:** `cycle_clusters` | **重要度:** ダイナミック (デフォルト)

循環依存クラスターは、循環依存が繁殖し始めたときに起こる現象です。単純な「AがBに依存、BがAに依存」というループではなく、複数のモジュールがすべて絡み合った複雑な網です。

デフォルトでは、重要度はクラスターのサイズによって決定されます：

- **0–5 ファイル**: 低 (Low)
- **6–15 ファイル**: 中 (Medium)
- **16–30 ファイル**: 高 (High)
- **31+ ファイル**: 致命的 (Critical)
