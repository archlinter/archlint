...
Это правило подсвечивает модули, которые нарушают главный закон чистой архитектуры: **стабильные части системы должны быть абстрактными.**

Оно ловит две архитектурные западни:

- **Зона боли (Zone of Pain)**: Модуль «слишком конкретный и слишком стабильный». От него все зависят, но его чертовски трудно менять, не ломая всё вокруг.
- **Зона бесполезности (Zone of Uselessness)**: Модуль «слишком абстрактный и слишком нестабильный». Вы построили красивый интерфейс, но его никто не использует. Классический пример оверхед-проектирования (overengineering).

Если кратко:  
👉 **Фундамент** системы должен быть **абстрактным** (интерфейсы/типы).  
👉 **Листья** системы должны быть **конкретными** (реализации).
