...
description: 'Узнайте, как исключить файлы или директории из анализа archlint, используя глобальное игнорирование, gitignore, исключения для конкретных правил и инлайновые комментарии.'
---
...
Секция `ignore` в корне `.archlint.yaml` исключает файлы из анализа. Игнорируемые файлы всё равно парсятся, поэтому импорты через них продолжают разрешаться, но смеллы для них не выводятся и они не учитываются ни в одном пороге детектора (например, `min_dependents` или `max_files_per_package`).
...
Паттерны сопоставляются с путями относительно корня проекта:

- Обычный путь — `legacy`, `src/legacy`, `generated.ts` — совпадает с самой записью и со всем, что внутри неё, на любой глубине.
- Паттерн, содержащий `*`, `?` или `[`, используется как glob. Учтите, что `*` совпадает и с `/`, поэтому `src/*.ts` покрывает `src/a/b/c.ts`.
- Непригодные паттерны выводятся как предупреждения и пропускаются; остальные паттерны продолжают действовать.

Единственное исключение из правила выше — циклы: цикл это факт о группе файлов, поэтому цикл, проходящий и через живые, и через игнорируемые файлы, всё равно выводится вместе с игнорируемыми участниками. Цикл, все участники которого игнорируются, не выводится.

Задание `ignore` **заменяет** встроенные значения по умолчанию (`**/*.test.ts`, `**/__tests__/**` и остальные паттерны тестовых файлов), а не дополняет их. Перечислите нужные заново, если тестовые файлы всё ещё должны исключаться.
