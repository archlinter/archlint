...
- **Слишком «бетонные» и слишком стабильные** — когда от конкретного класса зависит слишком много файлов (это трудно и опасно менять).
...
В `archlint` под **модулем** понимается один файл исходного кода (`.ts`, `.tsx`, `.rs` и т.д.), который экспортирует хотя бы один символ.
...
- **Проблема**: Все зависят от конкретной реализации. Менять её опасно, так как модуль одновременно жесткий и имеет слишком много связей.
