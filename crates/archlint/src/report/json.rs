use crate::detectors::{CycleCluster, Explanation, SmellType};
...
pub fn generate_json(
    ...
    let smells_json: Vec<_> = report
        .smells
        .iter()
        .map(|(smell, explanation)| build_smell_json(smell, explanation))
        .collect();

    let summary = build_summary_json(report, config);
...
fn format_smell_type(smell_type: &SmellType) -> String {
    match smell_type {
        ...
        SmellType::HighCyclomaticComplexity {
            ...
            format!("high_cyclomatic_complexity: {name} at line {line} (complexity: {complexity})")
        }
        SmellType::HighCognitiveComplexity {
            ...
            format!("high_cognitive_complexity: {name} at line {line} (complexity: {complexity})")
        }
        ...
        SmellType::FeatureEnvy { most_envied_module } => {
            format!("feature_envy: envies {}", most_envied_module.display())
        }
        ...
        SmellType::TestLeakage { test_file } => {
            format!("test_leakage: imports {}", test_file.display())
        }
        SmellType::LayerViolation {
            from_layer,
            to_layer,
        } => {
            format!("layer_violation: {from_layer} -> {to_layer}")
        }
...
fn build_location_json(loc: &crate::detectors::LocationDetail) -> serde_json::Value {
...
fn build_cluster_json(cluster: &CycleCluster) -> serde_json::Value {
    json!({
...
fn build_smell_json(
    ...
    let mut smell_json = json!({
        "type": format_smell_type(&smell.smell_type),
        "severity": format!("{:?}", smell.severity),
        "files": smell.files.iter().map(|p| p.to_string_lossy()).collect::<Vec<_>>(),
        "locations": smell.locations.iter().map(build_location_json).collect::<Vec<_>>(),
        "explanation": {
    ...
    if let Some(cluster) = &smell.cluster {
        smell_json["cluster"] = build_cluster_json(cluster);
    }
