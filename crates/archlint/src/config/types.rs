...
pub struct Config {
    ...
    /// Patterns that cannot be used are reported as warnings and skipped.
    ///
    /// Ignored files are still parsed so that imports resolve, but no smell is reported
    /// for them and they do not count towards any detector threshold. Cycles are the one
    /// exception: a cycle is a fact about a group of files, so one that runs through both
    /// live and ignored files is still reported, with its ignored members listed. A cycle
    /// whose members are all ignored is not reported.
    ///
