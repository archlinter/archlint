...
impl GitStorage {
    pub fn open(path: &Path) -> Result<Self> {
        ...
        {
            let _ = write_txn.open_table(COMMITS_TABLE).map_err(|e| {
    ...
    pub fn get_commit_data(&self, oid: &[u8; 20]) -> Result<Option<CommitData>> {
        let read_txn = self.db.begin_read().map_err(|e| {
            crate::AnalysisError::Storage(format!("Failed to begin read txn: {}", e))
        })?;
        let table = read_txn
            .open_table(COMMITS_TABLE)
            .map_err(|e| crate::AnalysisError::Storage(format!("Failed to open table: {}", e)))?;

        let value = table.get(oid).map_err(|e| {
            crate::AnalysisError::Storage(format!("Failed to get from table: {}", e))
        })?;
